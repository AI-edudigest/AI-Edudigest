...
function App() {
  ...
  const getPageContent = () => {
    switch (activeSection) {
      ...
      case 'resource':
        return activeResource ? <ResourcePage resourceType={activeResource} onGoBack={handleGoBack} canGoBack={navigationHistory.length > 0} /> : <HomePage onResourceClick={handleResourceClick} onGoBack={handleGoBack} canGoBack={navigationHistory.length > 0} />;
      default:
      ...
      <div className={`min-h-screen bg-gray-50 dark:bg-gray-900 flex transition-colors duration-200 ${isDarkMode ? 'dark' : ''}`}>
        ...
        <div className={`flex-1 flex flex-col min-h-screen transition-all duration-300 ${isCollapsed ? 'ml-16' : 'ml-72'} max-lg:ml-0`}>
          ...
          <div className="flex-1 flex min-h-0">
            ...
            {!isCollapsed && (
              <div className="hidden xl:block w-80 flex-shrink-0 fixed right-0 top-20 h-[calc(100vh-5rem)] z-30">
                <RightSidebar 
